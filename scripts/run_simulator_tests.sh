...
# marked test suite with TRNBLAS_USE_SIMULATOR=1 set in the SSM env.
#
# Still AWS-resident for now (the nki wheel is linux_x86_64 only + lives
# on the AWS pip index, not a common macOS target). Future follow-up:
# host simulator tests on GH Actions ubuntu-latest runners.
