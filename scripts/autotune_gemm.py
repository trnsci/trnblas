...
from trnblas.nki.dispatch import HAS_NKI, _round_up, _to_xla, _REQUIRE_NKI
...
def measure(
    kernel: Callable, A: torch.Tensor, B: torch.Tensor,
...
def main() -> None:
    ...
    ap.add_argument("--configs", type=int, default=None,
                    help="Limit to first N configs for smoke-test")
    args = ap.parse_args()

    if not HAS_NKI:
        print("HAS_NKI is False — NKI isn't available; tile configs are a no-op "
